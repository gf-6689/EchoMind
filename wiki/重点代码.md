...
```python
async def recognize(self, message: str, history=None, mode: str = "fusion") -> IntentResult:
...
| 模式 | 权重 |
|------|------|
| Fusion | LLM 70% + Embedding 20% + Pattern 10% |

### 本地 BGE Embedding

意图模板匹配使用本地 `BAAI/bge-small-zh-v1.5`。模板向量会缓存，编码时启用归一化；模型加载和编码通过线程卸载，避免阻塞事件循环。该 Embedding 分支与 ChromaDB 业务知识库的 embedding 配置不是同一条链路。

---

## 3. 本进程工具调用：缓存、熔断、fallback
